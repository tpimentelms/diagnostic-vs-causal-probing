...
the measure and an experimental protocol on the hierarchical equality task, and
report preliminary results: a linear probe recovers the task's intermediate variables
almost perfectly, while a linear DAS map does not recover them causally on the same
model. We discuss why, and what is needed to turn this into a clean comparison.
\end{abstract}
...
We report initial measurements on the hierarchical equality task. They are
preliminary: we have a clean diagnostic-probe baseline and a working DAS pipeline,
but the full complexity sweep (deeper probes and RevNet alignment maps) is not yet
run.

\paragraph{The intermediate variables are linearly decodable.} A linear probe on the
trained MLP's first-layer activations recovers $\textsc{wx}$ and $\textsc{yz}$ with
about $0.98$ and $0.96$ accuracy. Decodability decreases with depth (to about $0.75$
by the third layer), while the output $O$ becomes near-perfectly decodable only in
the deeper layers ($0.70 \to 0.997$). So at the layer where the two variables are
most linearly present, a probe finds them almost perfectly.

\paragraph{Linear DAS does not recover them causally.} Training a linear (rotation)
alignment map at that same layer to perform the $\textsc{wx}$ interchange yields
interchange-intervention accuracy at roughly chance ($\sim\!0.52$--$0.56$), even when
we train on the single, label-balanced $\textsc{wx}$ intervention in isolation. This
is not an optimisation failure of the obvious kind: the rotation receives a
non-trivial gradient and moves steadily away from its initialisation throughout
training; the accuracy simply does not improve. So on this model a feature that is
almost perfectly \emph{decodable} by a linear probe is not \emph{interveneable} by a
linear alignment map.

We are cautious about over-reading this DAS number. It is below the interchange
accuracy that linear DAS reaches on this task in \citet{sutter2025nonlinear}, which
points to a factor specific to our setup rather than a property of linear alignment
in general. The most likely culprit is that our trained MLP is badly calibrated: it
produces very large logits (we observed class-logit gaps of order $50$, with
cross-entropy of order $10$ under a random alignment), which makes the cross-entropy
landscape the rotation is optimised on sharp and hard to descend. Calibrating the
target model is the first thing to fix before reading anything into the DAS numbers.

\section{Implementation notes}\label{sec:limitations}

Two methodological points cost us the most time and are worth recording, as they
affect anyone running DAS on this kind of data.

\paragraph{Judge causal probes by held-out, order-independent evaluation.} The
obvious training-time metric---a running average of interchange accuracy---is
misleading here. The counterfactual data is generated in contiguous blocks that each
share one intervention type, and for the joint (both-subspace) intervention each block
turns out to be label-homogeneous. A running average over such blocks drifts with the
order the blocks happen to arrive in, which looks exactly like a model that ``learns
then collapses'' even when the alignment map is frozen. Every number above comes from
a single order-independent pass over a held-out set, broken down by intervention type.

\paragraph{The joint intervention is not informative as generated.} Because each
joint-intervention block is a single class, it provides no signal for learning the
equality, and its per-block accuracy merely tracks the block's label; we therefore do
not report it. Balancing labels within each block (or mixing blocks within a gradient
step) is needed before the joint intervention can be trained or evaluated
meaningfully. Two smaller pitfalls: a ``fast'' intervention path we used initially
keeps only the first of several source locations, silently dropping the second source
in the joint intervention; and, as noted above, the target model's miscalibration is
a likely cause of the poor DAS optimisation and has not yet been isolated.
...
hierarchical equality task show a sharp contrast on the causal side: the task's
intermediate variables are almost perfectly linearly decodable, yet a linear
alignment map does not recover them causally on the same model. We suspect this last
result is partly an artefact of a miscalibrated target model rather than a property
of linear alignment, and resolving that is the first step before the full complexity
sweep can be run. The payoff, if the sweep bears it out, is a concrete sense in which
causal probing is the safer tool at finite complexity---even though, in the limit, it
is no better than the diagnostic probing it was meant to improve on.
