...
\begin{abstract}
\noindent
Diagnostic probing and causal probing (via distributed alignment search, DAS) 
are supervised interpretability techniques, both of which
tend to find what one looks for: (i)
a sufficiently powerful diagnostic probe
always recovers a target property from a model's representations \citep{pimentel2020information}; and a
sufficiently complex causal probe always recovers an alignment between any algorithm and a 
neural network \citep{sutter2025nonlinear}. Both these statements, however, are about a limit: 
at infinite complexity, either method can fit any target, including random ones, so 
their reported scores say nothing about a neural network.
Read at face value, these results say that causal probing is no
better than diagnostic probing---it leaves us where diagnostic probing did. But we only know 
this is true at the infinite complexity limit. In practice, we do not work at that limit, 
and at finite complexity and finite data, the extra structure in a causal
probe might make it more resistant to fitting \emph{spurious} targets. This report proposes
...
interveneable. A first sweep over dataset size finds DAS fitting random relabellings no
more readily than the probe---and clearly less so at the most informative size---while
matching it on signal: a preliminary, single-seed sign that causal probing is the safer
...
Section~\ref{sec:setup} assumptions, a contextual representation contains exactly as
...
This quantity is, up to a scaling constant and the usual move from a
...
Definition~\ref{def:capacity} in accuracy space rather than as the textbook
...
Definition~\ref{def:capacity} is not what the two theorems are about, and the difference
...
grows differently as we make each more complex. Definition~\ref{def:capacity} measures
...
\emph{randomised} targets, Definition~\ref{def:capacity}). Each method traces a curve in
...
because the ground-truth intermediate variables (WX, YZ) are
...
$\widehat{\rade}_n(\hypclass_{\complexity})$ of Definition~\ref{def:capacity}, estimated
...
\emph{signal--spurious plane} of Section~\ref{sec:measure}; and (3) fit a simple
...
the trained MLP's first-layer activations recovers WX with about $0.98$
...
map trained at that same layer to perform the WX interchange reaches about
$0.999$ interchange-intervention accuracy. So, on the correct labels, both methods
recover WX, and recover it strongly: the feature is both decodable by a
...
single WX intervention (balanced batches, one objective) makes the rotation
...
\paragraph{Spurious-fitting capacity.} Figure~\ref{fig:scaling} runs the sweep of
Section~\ref{sec:protocol}: train-set fit against $n$, on the correct labels and on a fixed
random relabelling, for both methods. On the correct labels both fit WX
essentially perfectly across the range (the probe at $1.0$; DAS at $\approx 1.0$ for
$n \gtrsim 300$). On random labels both start at $1.0$ at the smallest $n$---any method
can memorise $30$ points---and both decay toward chance by $n \approx 2\times10^{4}$.
In between, where capacity rather than data is the binding constraint, DAS's
random-label fit sits at or below the probe's at most sizes, most clearly at $n=90$
(probe $\approx 0.86$, DAS $\approx 0.66$). Taken at face value, DAS reaches the same
signal while fitting noise less readily---its spurious-fitting capacity is the smaller
of the two---which is the direction the finite-complexity hypothesis predicts. We
report this as suggestive, not settled: the curves are single-seed and noisy (they
cross once, at $n=300$, and DAS's correct-label fit wobbles at the smallest $n$), so the
gap should be confirmed by averaging over seeds before it is leaned on.
...
(spurious capacity, dashed). Both recover WX on the correct labels; on random
labels both fall from perfect memorisation at small $n$ toward chance, and DAS's curve
lies at or below the probe's at most sizes (most clearly at $n=90$), indicating lower
spurious-fitting capacity. Single seed.}
\label{fig:scaling}
\end{figure}

\paragraph{What we did not do.} Two extensions would turn this into a clean result:
averaging over seeds (with error bands) to confirm the gap, and sweeping a genuine
\emph{complexity} axis (deeper probes; RevNet alignment maps) rather than dataset size.
...
\paragraph{Why a single intervention.} We restrict to the WX interchange.
...
converge (Section~\ref{sec:results}).
...
Finally, a caveat on what Definition~\ref{def:capacity} measures. It is about fitting random
...
setup, not a property of linear alignment. With both recovering the signal, the comparison
that matters is spurious-fitting capacity, and a first (single-seed) sweep over dataset
size points the predicted way: DAS fits random labels no more readily than the probe,
and less readily at the most informative size. If this survives seed-averaging and a
genuine complexity axis, it is a concrete sense in which causal probing is the safer
tool at finite complexity---even though, in the limit, it is no better than the
diagnostic probing it was meant to improve on.
